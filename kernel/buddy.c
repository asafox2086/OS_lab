...
#define NPAIR(k)      ((NBLK(k)+1)/2)            // Number of buddy pairs at size k
...
bd_print() {
  for (int k = 0; k < nsizes; k++) {
    ...
    printf("  alloc:");
    bd_print_vector(bd_sizes[k].alloc, NPAIR(k));
    if(k > 0) {
      printf("  split:");
...
bd_malloc(uint64 nbytes)
{
  ...
  bit_flip(bd_sizes[k].alloc, pair_index(blk_index(k, p)));
  for(; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k-1);   // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    bit_flip(bd_sizes[k-1].alloc, pair_index(blk_index(k-1, p)));
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_flip(bd_sizes[k].alloc, pi);  // free p at size k
    if (merge == 0) {  // is buddy allocated?
...
bd_initfree_pair(int k, int bi, int free_bi) {
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
  int free = 0;
  if(bi < 0 || bi >= NBLK(k) || buddy < 0 || buddy >= NBLK(k) ||
     free_bi < 0 || free_bi >= NBLK(k))
    return 0;
  if(bit_isset(bd_sizes[k].alloc, pair_index(bi))) {
    // one of the pair is free
    free = BLK_SIZE(k);
    lst_push(&bd_sizes[k].free, addr(k, free_bi));   // put the free buddy on free list
  }
  return free;
}
...
bd_initfree(void *bd_left, void *bd_right) {
  ...
  for (int k = 0; k < MAXSIZE; k++) {   // skip max size
    ...
    if(right <= left)
      continue;
    int right_buddy = (right % 2 == 0) ? right+1 : right-1;
    free += bd_initfree_pair(k, right, right_buddy);
  }
  return free;
}
...
bd_init(void *base, void *end) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    sz = sizeof(char)* ROUNDUP(NPAIR(k), 8)/8;
    bd_sizes[k].alloc = p;
